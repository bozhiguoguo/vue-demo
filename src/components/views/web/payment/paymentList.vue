        ...
        <div class="tableBox">
          <el-table
            ...
            @selection-change="handleSelectionChange">
              ...
              <el-table-column prop="defaultAmendsMoney" label="保险限额" header-align="center">
                <template slot-scope="scope">
        ...
        <el-table :data="dialogTableData">
          <el-table-column label="保险类型" header-align="center">
            <template  slot-scope="scope">
              ...
              <span v-if="scope.row.productType==5">综合保险业务</span>
            </template>
          </el-table-column>
    ...
    export default {
        ...
        data() {
            return {
                ...
                tableData: [],
                checkedCities: [],
                tabData: [{"name": "全部", "id": ""}, {"name": "商标综合保险", "id": "5"}, {
                    "name": "果保专利保险",
                    "id": "4"
                }, {"name": "盲查保险", "id": "1"},],
                formInline: {
                    ...
                    "productType": "",//	是	string	保险产品类型  5 商标综合保险     4 果保专利保险       1 盲查保险
                    "pageParam": {
                        "pageNo": 1,
                        "pageSize": 10
                    },
                },
                header: ["全选", "保险编号", "订单编号", "申请号", "投保日期", "报案日期", "被保险人名称", "订单状态", "保费", "保险金额"],
                data_list: [
                    [1, 2, 3],
                    [4, 5, 6]
                ],
                ...
                exporeExcelObject: {
                    ...
                    chArray: ['保险编号', '订单编号', '申请号', '投保日期', '报案日期', '被保险人名称', '保险类型', '订单状态', '保费', '保险限额'],
                    enArray: ['inseCode', 'ordercode', 'applyCode', 'devoteInsuTime', 'reportCaseTime', 'bodyName', 'productType', 'status', 'insuMoney', 'defaultAmendsMoney'],
                    importFlag: true
                },
                dialogTableData:[],
                allMoney:0,
        ...
        methods: {
            ...
            loadlist(type) {
                ...
                //列表数据
                this.$post(this.$api + '/insuOrder/getInsuOrderListByPageAndAmendsForInsu', self.formInline).then((res) => {
            ...
            throughClaims() {
              ...
              this.$post(this.$api + 'insuOrder/getInsuOrderPayCont/', {amendsIds: strData}).then((res) => {
                  if (res.success && res.code == 0) {
                    console.log(res);
            ...
            //支付成功确定
            claimsSure(){
                this.$put(this.$api + '/insuOrder/updateInsuPaystatus', {amendidList: this.checkedCities}).then((res) => {
                  if (res.success && res.code == 0) {
                    this.loadlist();
                    this.checkAll=false;
                  } else {
